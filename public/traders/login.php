...
if(is_post_request()) {
  ...
  if(empty($errors)) {
    ...
    $trader = find_trader_by_username($username);
    if($trader) {

      if ($password === $trader['password']){
        log_in_trader($trader);
        redirect_to(url_for('/traders/index.php'));
      } else {
...
<title> Login </title>
  ...
  <div class="user_welcome">Σύνδεση Εμπόρου</div>
  <?php echo display_session_message(); ?>
<section  class="content">

<form class="login" action="<?php echo url_for('/traders/login.php');?>" method = "post">
